...
@Component({
  selector: 'app-training-item-detail',
  templateUrl: './training-item-detail.component.html',
  styleUrls: ['./training-item-detail.component.scss']
})
export class TrainingItemDetailComponent extends BaseImageUpload {
  ...
  ngOnInit(): void {
    ...
    if (this.id) {
      this._trainingItemSrv.getById(this.id).subscribe((item) => {
  ...
  create() {
    this._trainingItemSrv
      ...
      .subscribe(() => {
        ...
        this.router.navigate(['/', 'training-item']);
      });
  }
